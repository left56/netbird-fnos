...
<template><FnPageHeader title="日志与诊断" description="最近 100 行（敏感字段已隐藏）"><template #default><FnButton @click="load">刷新</FnButton><FnButton @click="copy">复制</FnButton><FnButton variant="primary" @click="download">下载</FnButton></template></FnPageHeader><FnCard><p v-if="error" class="error">{{error}}</p><pre v-else>{{lines.length ? lines.join('\n') : '暂无日志。'}}</pre></FnCard></template>

...
const message = ref(''); const version = ref(''); const sourceID = ref(''); const file = ref<HTMLInputElement>(); const source = ref({name:'',enabled:true,priority:10,urlTemplate:'',timeoutSeconds:60})
...
async function upload(){const selected=file.value?.files?.[0];if(!selected)return;const body=new FormData();body.append('file',selected);body.append('allowUnverified','false');try{await api('/api/client/upload',{method:'POST',body});message.value='上传已验证并安装。';await refresh()}catch{message.value='上传被拒绝：未校验上传必须明确高级确认。'}}
...
<template>
  <section><h2>客户端管理</h2><p>管理内置与在线安装的官方 NetBird CLI。绝对路径不会暴露或可编辑。</p><p v-if="message">{{ message }}</p>
    ...
    <h3>在线安装</h3><p>只接受指定官方 Release 版本；最终依据官方 checksum 校验。</p><input v-model="version" placeholder="例如 0.71.4"><select v-model="sourceID"><option value="">官方源</option><option v-for="s in sources.filter(s=>s.enabled)" :key="s.id" :value="s.id">{{s.name}}</option></select><button @click="download">下载并安装</button>
    <h3>手动上传</h3><p>接受官方 ELF 或官方 tar.gz。未能由官方 checksum 验证的上传需要高级确认，默认不会激活。</p><input ref="file" type="file" accept="application/gzip,.gz,application/octet-stream"><button @click="upload">验证并安装</button>
